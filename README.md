...
```bash
curl -LO https://github.com/sreenathmmenon/krelvan/releases/download/v0.1.3/docker-compose.release.yml
...
Download `krelvan-0.1.3.tgz`, `SHA256SUMS`, and the attestations from the
[v0.1.3 release](https://github.com/sreenathmmenon/krelvan/releases/tag/v0.1.3).
After verification:

```bash
npm install --global ./krelvan-0.1.3.tgz
krelvan
```
...
npm test             # all tests pass (1 requires Ollama and skips without it)
...
**Built & verified** (typecheck clean · all tests pass · web build green):
